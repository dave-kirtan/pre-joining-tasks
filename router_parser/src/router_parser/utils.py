    ...
    with open(file_path, mode="r", encoding="utf-8") as file:
        ...
        for row_n, row in enumerate(reader, start=2):
            ...
            if not hostname or not check_hostname(hostname):
                print(
                    f"{Fore.RED}[ERROR] Line {row_n}: Invalid hostname (must use lowercase letters, numbers, and hyphens){Style.RESET_ALL}"
                )
                continue
            ...
            if not check_ipv4_address(ip_address):
                print(
                    (
                        f"{Fore.RED}[ERROR] Line {row_n}: Invalid IPv4 address{Style.RESET_ALL}"
                    )
                )
                continue
            ...
            if not model and not site:
                print(
                    (
                        f"{Fore.RED}[ERROR] Line {row_n}: Missing 'model' and 'site' fields{Style.RESET_ALL}"
                    )
                )
                continue
            ...
            if not model or not site:
                print(
                    f"{Fore.YELLOW}[WARNING] Line {row_n}: Skipped. Missing required data (Model: '{model}', Site: '{site}').{Style.RESET_ALL}"
                )
                continue
